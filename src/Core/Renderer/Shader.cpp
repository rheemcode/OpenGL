...
void Shader::UploadUniformMat4(const std::string& name, const Matrix4x4& p_mat4)
{
	GLint location = glGetUniformLocation(program, name.c_str());
	glUniformMatrix4fv(location, 1, GL_FALSE, Matrix4x4::GetAsFloatPtr(p_mat4));
}

void Shader::UploadUniformVec4(const std::string& name, const SimpleVec4& p_vec4)
{
	GLint location = glGetUniformLocation(program, name.c_str());
	glUniform4f(location, p_vec4.x, p_vec4.y, p_vec4.z, p_vec4.w);
}

void Shader::UploadUniformVec3(const std::string& name, const Vector3& p_vec3)
{
	GLint location = glGetUniformLocation(program, name.c_str());
	glUniform3f(location, p_vec3.x, p_vec3.y, p_vec3.z);
}

void Shader::UploadUniformVec2(const std::string& name, const Vector2& p_vec2)
{
	GLint location = glGetUniformLocation(program, name.c_str());
	glUniform2f(location, p_vec2.x, p_vec2.y);
}

void Shader::UploadUniformInt(const std::string& name, int p_val)
{
	GLint location = glGetUniformLocation(program, name.c_str());
	glUniform1i(location, p_val);
}

void Shader::UploadUniformFloat(const std::string& name, float p_val)
{
	GLint location = glGetUniformLocation(program, name.c_str());
	glUniform1f(location, p_val);
}

void Shader::UploadUniformIntArray(const std::string& name, int* p_val, uint32_t count)
{
	GLint location = glGetUniformLocation(program, name.c_str());
